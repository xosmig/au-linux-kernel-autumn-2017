...
static long vsd_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    switch (cmd) {
    case VSD_IOCTL_GET_SIZE:
        return vsd_ioctl_get_size((vsd_ioctl_get_size_arg_t*) arg);
    case VSD_IOCTL_SET_SIZE:
        return vsd_ioctl_set_size((vsd_ioctl_set_size_arg_t*) arg);
    default:
        return -ENOTTY;
    }
}
...
static int vsd_driver_probe(struct platform_device *pdev)
{
    ...
    vsd_dev = (vsd_dev_t*)devm_kzalloc(&pdev->dev, sizeof(*vsd_dev), GFP_KERNEL);
    if (!vsd_dev) {
        ret = -ENOMEM;
...
error_misc_reg:
    // The memory will be automatically freed (I hope)
    // kfree(vsd_dev);
    // vsd_dev = NULL;
error_alloc:
    return ret;
